# Algemene Informatie
Dit is de grondwet van VuceLife, deze dient door alle spelers te worden gevolgd.
Indien deze grondwet niet wordt gevolgd door één of meerdere personen, zal er een gepaste sanctie volgen vanuit het staffteam.

# Artikel 1 - Basiswetten
## Artikel 1.1 - Algemene wetten op ale platformen

> **Artikel 1.1.1;** Vertoon respect
>> Respecteer iedereen.

> **Artikel 1.1.2;** Zelfpromotie
>> Adverteren voor andere organisaties is niet toegestaan. Advertenties met betrekking tot VuceLife,
>> zoals onze website is wel toegestaan.

> **Artikel 1.1.3;** Pestgedrag
>> Het pesten en- of beledigen van andere mensen wordt niet getolereerd en is niet toegestaan.

> **Artikel 1.1.4;** Discriminatie
>> Discriminatie is ten strengste verboden op VuceLife. Iedereen moet gelijk behandeld worden.

> **Artikel 1.1.5;** Wanordelijk gedrag
>> Ongepaste of seksueel getinte spraak, tekst, beeld, etc. Is niet toegestaan op al onze platformen.

> **Artikel 1.1.6;** Alt-Accounts
>> Het gebruik van alt-accounts is alleen toegestaan met toestemming van een admin.

> **Artikel 1.1.7;** Straffen ontlopen
>> Het ontlopen van straffen (door bijvoorbeeld gebruik van een VPN of alt-account) is niet toegestaan. 

> **Artikel 1.1.8;** Bugs en fouten
>> Als je een bug of glitch tegenkomt, zouden wij het op prijs stellen dat je deze rapporteert. Dit kan via een ticket.
>> Misbruik maken van een bug of een glitch is ten strengste verboden.

> **Artikel 1.1.10;** Away From Keyboard
>> Wanneer je voor een tijd AFK (away from keyboard) bent, maak jezelf onhoorbaar voor anderen op discord of verlaat het kanaal.
>> Wanneer je AFK in-game bent, dien je een emote (animatie) uit te voeren zoals bijvoorbeeld leunen tegen een muur.
>> Hou er rekening mee dat je na een tijd niet bewegen in-game je uit de server wordt verwijderd.
>> Het is niet toegestaan om het AFK-Kick systeem te ontlopen.

> **Artikel 1.1.11;** Muziek
>> Voor het afspelen van muziek in een niet-privékanaal moet de muziek bot gebruikt worden.
>> Voor het gebruik van de muziek bot is de DJ rol benodigd. Het is niet toegestaan om ongepaste muziek af te spelen.

> **Artikel 1.1.12;** Soundboard
>> Het gebruik van een soundboard is niet toegestaan, tenzij een admin hiervoor toestemming geeft.

> **Artikel 1.1.13;** Voice changer
>> Het gebruik van een voice changer is niet toegestaan.

> **Artikel 1.1.14;** Opnamens
>> Het is niet toegestaan om op te nemen in een spraakanaal op de discord server zonder toestemming van een admin.

> **Artikel 1.1.15;** Taggen
>> Het taggen van rollen, @everyone en @here is niet toegestaan, met uitzondering van admins.

> **Artikel 1.1.16;** Commands
>> Commands mogen alleen worden gebruikt in de daarvoor aangewezen kanalen.

> **Artikel 1.1.17;** Chatgebruik
>> In onze discord server is het niet toegestaan om te spammen. Ook het overmatig gebruik van emoji's en CAPSLOCK zijn niet toegestaan.

# Artikel 2 - Wetten gerelateerd aan FiveM (ingame)
## Artikel 2.1 - Roleplaygerichte wetten

> **Artikel 2.1.1;** Random deathmatch (RDM)
>> Het vermoorden of aanvallen van andere spelers zonder dat hierbij een geldige (voorafgaande) reden is in roleplay,
>> is niet toegestaan.

> **Artikel 2.1.2;** Vehicle Deathmatch (VDM)
>> Het aanrijden en/of doden van spelers met een voertuig is niet toegestaan zonder een geldige reden in de roleplay.

> **Artikel 2.1.3;** Onrealistisch rijgedrag
>> Onrealistisch rijgedrag is niet toegestaan. Blijf dus op de wegen en rij niet door na een crash.

> **Artikel 2.1.4;** Uitloggen in roleplay (Combatloggen)
>> Wanneer je weg moet tijdens een roleplay-scenario, moet je dit scenario eerst op een goede manier afronden.
>> Is je game je gecrasht? Stuur dan even een bericht in een OOC kanaal op de discord.

> **Artikel 2.1.5;** Vliegen
>> Je mag enkel vliegen wanneer je een vliegbrevet hebt.

> **Artikel 2.1.6;** Powergaming
>> Powergaming is niet toegestaan.

> **Artikel 2.1.7;** Waardeer je leven
>> Waardeer je leven en dat van een ander. Gedraag je niet te moordlustig en probeer je eigen leven te beschermen.

> **Artikel 2.1.8;** Breken van karakter
>> Blijf ten alle tijden in je karakter wanner je in-game zit. 

> **Artikel 2.1.9;** Metagaming
>> Gebruik in de roleplay geen informatie die je niet in de roleplay hebt opgedaan.

> **Artikel 2.1.10;** New life rule
>> Wanneer je in een roleplay-situatie doodgaat, mag je niet meer meedoen aan deze situatie.

## Artikel 2.2 - Skins en uiterlijk

> **Artikel 2.2.1;** Standaard skin
>> Wanneer je de server voor het eerst joined, moet je je skin aanpassen.

> **Artikel 2.2.2;** Verboden kleding
>> Het dragen van kleding van een overheidsinstantie, zoals de politie of DAR, is niet toegestaan.

## Artikel 2.3 - Voice Chat

> **Artikel 2.3.1;** Roleplay gerichtheid
>> Alles wat in de voice chat wordt gezegd kan in roleplay verwerkt worden.
>> Je kan bijvoorbeeld een bekeuring krijgen voor geluidsoverlast.

> **Artikel 2.3.2;** Muziek
>> Muziek afspelen is enkel toegestaan wanneer dit in roleplay mogelijk is.
>> Denk hierbij aan een radio animatie. Zingen mag.

> **Artikel 2.3.3;** Soundboard
>> Het gebruik van een soundboard is enkel toegestaan wanneer dit in roleplay past.
>> Denk hierbij aan het gebruik van een passende animatie.

> **Artikel 2.3.4;** Voicechanger
>> Het gebruik van een voicechanger is niet toegestaan mits dit mogelijk is in roleplay, bijvoorbeeld via een telefoon.
>> Je mag natuurlijk wel zelf een stemmetje opzetten of met een accent praten.

## Artikel 2.4 - Opname

> **Artikel 2.4.1;** - Opnemen en streamen
>> Opnemen en streamen is toegestaan op de FiveM server. Hou hier als speler rekening mee.
>> Stream-snipen geldt als meta-gaming (artikel 2.1.9).

## Artikel 2.5 - Meldingsysteem

> **Artikel 2.5.1;** Hulpdiensten
>> De hulpdiensten zoals ambulance, brandweer, politie, ... contacteer je met de volgende commands:
>> **Politie:** */112 (tekst)*
>> **Reddingsdiensten:** */113 (tekst)*

> **Artikel 2.5.2;** Technische diensten
>> De technische diensten zoals de wegenwacht, contacteer je met de onderstaande commands:
>> **Wegenwacht:** *(NADER TE BEKIJKEN)*

> **Artikel 2.5.3;** Roleplay
>> Gebruik het meldingssyteem enkel wanneer dit in roleplay mogelijk en nodig is.

> **Artikel 2.5.4;** Misbruik meldingsysteem
>> Het meldingsysteem voor de hulpdiensten mag niet misbruikt worden.
>> Via het meldingsysteem contacteer je geen staff lid!

## Artikel 2.6 - Criminaliteit

> **Artikel 2.6.1;** Vluchten
>> Vluchten van de politie mag enkel met een geldige reden.

> **Artikel 2.6.2;** Vluchtvoertuig
>> Wanneer een voertuig een belangrijke rol in een scenario heeft, zoals een achtervolging.
>> Mag deze niet in de garage gezet worden tot het scenario volledig is afgerond.

> **Artikel 2.6.3;** Overval
>> Het is niet toegestaan om meerdere overvallen in korte tijd te plegen.
>> Er moeten minimaal drie agenten actief zijn.

> **Artikel 2.6.4;** Criminele organisaties
>> Criminele organisaties krijgen op VuceLife geen speciale rollen en/of voordelen. 
>> Alle criminele organisaties moeten in roleplay gevormd worden.

> **Artikel 2.6.5;** Gijzeling
>> Voor een gijzeling moet een geldige reden zijn in roleplay.
>> Gijzel dus geen random spelers.

> **Artikel 2.6.6;** Diefstal van voertuigen
>> Het is niet toegestaan om bedrijfsvoertuigen te stelen (voertuigen van slager, textiel, ...).
>> Overheidsvoertuigen mogen worden gestolen, met uitzondering van voertuigen van de ambulancediensten
>> en de wegenwacht. Je mag ook geen spullen uit bedrijfsvoertuigen stelen.

> **Artikel 2.6.7;** Geheime locaties
>> Wees subtiel met het delen van geheime locaties. Wanneer te veel mensen van deze locaties weten, zullen deze verplaatst worden.

> **Artikel 2.6.8;** Vuurwapens
>> Wees realistisch met je vuurwapen. Schieten vanuit een voertuig is niet toegestaan.

> **Artikel 2.6.9;** Advocaten
>> Als je een advocaat wilt zijn, moet je dit officieel laten registreren door een ticket aan te maken.

## Artikel 2.7 - Overheidsbanen

> **Artikel 2.7.1;** (Cop) Bating
>> Het uitlokken van hulpdiensten is niet toegestaan. Je mag ook niet de hulpdiensten achterna rijden, tenzij ze hierom vragen.

> **Artikel 2.7.2;** Materiaal
>> Het doorverkopen en delen van hulpdiensten meterialen zoals wapens in ten strengste verboden.

> **Artikel 2.7.3;** Veiligheidsgarantie
>> Ambulancediensten en wegenwachters mogen niet worden aangevallen, beroofd, gegzijzeld, ... wanneer zij in dienst zijn.
>> Deze regel is niet van toepassing wanneer de ambulancier / wegenwachter zichzelf verwikkeld in een crimineel scenario.

> **Artikel 2.7.4;** Opendeurdagen
>> Wanneer er een opendeurdag is georganiseerd, is de kazerne/post en de directe omgeving hiervan een safezone.
>> Er mag hier dus geen criminaliteit plaatsvinden.

> **Artikel 2.7.8;** Slepen/Lockpicken van voertuigen
>> Een voertuig mag pas gesleept/gelockpickt worden door de overheid wanneer de eigenaar van het voertuig
>> of een bevoegd politieambtenaar hier specifiek toestemming voor geeft.

> **Artikel 2.7.9;** Oproepen
>> Wanneer een overheidsinstantie een oproep maakt naar een andere overheidsinstantie,
>> krijgt deze oproep prioriteit boven de andere meldingen. Als er nog een melding bezig is,
>> moet er een inschatting worden gemaakt welke belangrijker is.

> **Artikel 2.7.10;** Ziekenhuisopname
>> Indien je behandeld wordt door een ambulancier, dien je naar deze persoon te luisteren.
>> Hij/zij bepaald wanneer je het ziekenhuis mag verlaten en of je je roleplay voort kan zetten.

> **Artikel 2.8.10;** Corruptie
>> Leidinggevenden van een overheidsinstantie mogen niet corrupt zijn.

# Artikel 3 - Report systeem

> **Artikel 3.1;** In-game report
>> (NADER TE BEKIJKEN)

> **Artikel 3.2;** Discord Report
>> Als je iemand in discord wilt rapporteren, kan je een ticket aanmaken.

> **Artikel 3.3;** Misbruik 
>> Misbruik maken van het report systeem, door bijvoorbeeld het maken vaneen valse report, is niet toegestaan.
>> Vragen stellen via een report is niet toegestaan. Voor vragen kan je een ticket aanmaken of een bericht sturen in het hulpkanaal op onze discord.

> **Artikel 3.4;** Beeld- en audiomateriaal
>> Als je beeld- of audiomateriaal bij je report wilt delen, dien je een ticket aan te maken.

# Artikel 4 - Donaties

> **Artikel 4.1;** Algemene Voorwaarden
>> De algemene voorwaarden voor donaties zijn te vinden op onze tebex pagina.

# Artikel 5 - Politiek
*Alle onderstaande regels onder artikel 5 zijn momenteel nog niet van toepassing. Dit aangezien deze mogelijkheid nog niet toegevoegd is aan VuceLife.*

# Belangrijke informatie
Wij van VuceLife zijn gemachtigd om deze regels op elk moment aan te passen. Indien er een aanpassing is, wordt dit meegedeeld.
Wanneer deze regels worden overtreden, zal het staff team een gepaste sanctie toepassen.
Wanneer u in onze discord en/of fivem server zit, gaat u automatisch akkoord met deze regels.